...
export function CourseSidebar() {
  ...
  const [expandedSet, setExpandedSet] = useState<Set<string>>(() => {
    ...
    return new Set(current ? [current] : ['01-architecture-basics'])
  })
  ...
  useEffect(() => {
    try {
      ...
      setExpandedSet(new Set(
        arr.length > 0 ? arr : (current ? [current] : ['01-architecture-basics']),
      ))
    } catch {
      setExpandedSet(new Set(['01-architecture-basics']))
    }
  }, [pathname])
...
function ExpandedHeader({
  ...
  return (
    <div className="flex items-center justify-between px-4 pt-4">
      <Link
        href="/learn"
        className="text-xs font-mono uppercase tracking-[0.2em] text-muted-foreground/70 hover:text-foreground"
      >
        ← К курсу
      </Link>
      <button
        ...
        aria-label={mode === 'mobile' ? 'Закрыть меню' : 'Свернуть меню курса'}
      >
  ...
  return (
    <div className="flex flex-1 flex-col items-center py-3">
      <button
        ...
        aria-label="Развернуть меню курса"
        title="Развернуть меню курса"
      >
        <ChevronsRight className="h-4 w-4" />
      </button>

      <nav className="flex flex-1 flex-col items-center gap-1 overflow-y-auto" aria-label="Модули курса">
        {COURSE_MODULES.map(module => {
