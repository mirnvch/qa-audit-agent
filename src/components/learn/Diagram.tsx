...
// ─── Inline-markdown для caption ────────────────────────────────────────────
// Поддерживает **bold**, *italic*, `code`. Без сторонних либ — для caption
// этого достаточно, MDX-grade парсер был бы overkill.

function renderInlineMarkdown(text: string): React.ReactNode {
  const tokens: React.ReactNode[] = []
  // (?:...)+ — alternation: bold | italic | code; capture group захватывает всё совпадение целиком.
  const re = /(\*\*[^*]+\*\*|\*[^*]+\*|`[^`]+`)/g
  let last = 0
  ...
  while ((match = re.exec(text)) !== null) {
    ...
    const m = match[0]
    if (m.startsWith('**')) {
      tokens.push(<strong key={key++} className="font-semibold text-foreground">{m.slice(2, -2)}</strong>)
    } else if (m.startsWith('`')) {
      tokens.push(<code key={key++} className="rounded bg-muted/40 px-1 py-0.5 font-mono text-[0.85em]">{m.slice(1, -1)}</code>)
    } else {
